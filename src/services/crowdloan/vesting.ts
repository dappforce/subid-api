...
import getVestedData from './getVestedData'
...
export const getCrowdloanVestingByAccount = async ({
  account,
  networks,
  noCache,
  apis
}: GetCrowdloanClaimableByAccountProps): Promise<MappedVestingData> => {
  const promises = networks.map(async (network) => {
    ...
    try {
      ...
      const balances = await getVestedData({ api, account })
      const zero = new BN(0)
      ...
      log.error(JSON.stringify(err.stack), network)
      return null
    }
  })
